...
subset_matrix <- AggregateExpression(object = integrated.obj, group.by = c("Class1", "Patient_id", "Timepoint"), slot = "counts")

metadata <- integrated.obj@meta.data %>%
  group_by(Patient_id, Class1, Timepoint) %>%
  summarise(n = n()) %>%
  mutate(Id = paste(Class1, Patient_id, Timepoint, sep = "_")) %>%
  as.data.frame()
rownames(metadata) <- metadata$Id

metadata <- metadata[order(match(rownames(metadata), colnames(subset_matrix$RNA))), ]

metadata$Class1 <- ifelse(metadata$Class1 == "CAR-", "CARneg", "CARpos")
...
# Filter by Peak
integrated.obj.subset <- subset(x = integrated.obj, subset = Timepoint == "Peak")

subset_matrix <- AggregateExpression(object = integrated.obj.subset, group.by = c("Class1", "Patient_id"), slot = "counts")

subset_metadata <- integrated.obj@meta.data %>%
  group_by(Patient_id, Class1) %>%
  summarise(n = n()) %>%
  mutate(Id = paste(Class1, Patient_id, sep = "_")) %>%
  as.data.frame()
...
subset_metadata$Class1 <- ifelse(subset_metadata$Class1 == "CAR-", "CARneg", "CARpos")
...
DESeq2::plotPCA(rlog, intgroup = "Class1", ntop = 500)
DESeq2::plotPCA(rlog, intgroup = "Patient_id", ntop = 500)
DESeq2::plotPCA(rlog, intgroup = "Sex", ntop = 500)


library(sva)
adjusted <- ComBat_seq(counts(dds), batch = dds$Patient_id, group = dds$Class1)
...
res <- results(dds, contrast = c("Class1", "CARpos", "CARneg"))
...
ann$Analisis <- "All"
write.csv(ann, "ann_Peak.csv", row.names = FALSE)
```
...
col_ha <- HeatmapAnnotation(CAR = dds$Class1, Patient = dds$Patient_id)
...
gene_both <- list("neg" = names(gene_list_neg), "pos" = names(gene_list_pos))
...
# Filter by Peak
integrated.obj.subset <- subset(x = integrated.obj, subset = final_criteria == "CD4" & Timepoint == "Peak")
...
ann$Analisis <- "CD4"
write.csv(ann, "ann_Peak_CD4.csv", row.names = FALSE)
```
...
# Filter by Peak
integrated.obj.subset <- subset(x = integrated.obj, subset = final_criteria == "CD8" & Timepoint == "Peak")
...
ann$Analisis <- "CD8"
write.csv(ann, "ann_Peak_CD8.csv", row.names = FALSE)
```
...
results <- lapply(levels(integrated.obj$seurat_clusters), function(cluster) {
  # Filter by Peak
  integrated.obj.subset <- subset(x = integrated.obj, subset = Timepoint == "Peak" & seurat_clusters == cluster)

  subset_matrix <- AggregateExpression(object = integrated.obj.subset, group.by = c("Class1", "Patient_id", "seurat_clusters"), slot = "counts")

  subset_metadata <- integrated.obj.subset@meta.data %>%
    group_by(Patient_id, Class1, seurat_clusters) %>%
    summarise(n = n()) %>%
    mutate(Id = paste(Class1, Patient_id, seurat_clusters, sep = "_")) %>%
    as.data.frame()
  rownames(subset_metadata) <- subset_metadata$Id
  ...
  Deg <- results(dds, contrast = c("Class1", "CARpos", "CARneg"))
...
write.csv(ann, "ann_Peak_Clusters.csv", row.names = FALSE)
```
