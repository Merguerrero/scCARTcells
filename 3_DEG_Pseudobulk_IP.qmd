...
metadata <- integrated.obj@meta.data %>%
  dplyr::group_by(Patient_id, Class1, Timepoint) %>%
  dplyr::summarise(n = n()) %>%
  dplyr::mutate(Id = paste(Class1, Patient_id, Timepoint, sep = "_")) %>%
  as.data.frame()
...
# Filter by IP
integrated.obj.subset <- subset(x = integrated.obj, subset = Timepoint == "IP")
...
subset_metadata <- integrated.obj@meta.data %>%
  dplyr::group_by(Patient_id, Class1) %>%
  dplyr::summarise(n = n()) %>%
  dplyr::mutate(Id = paste(Class1, Patient_id, sep = "_")) %>%
  as.data.frame()
...
ann$Analisis <- "All"
write.csv(ann, "ann_IP.csv", row.names = FALSE)
```
...
# Filter by IP
integrated.obj.subset <- subset(x = integrated.obj, subset = final_criteria == "CD4" & Timepoint == "IP")
...
ann$Analisis <- "CD4"
write.csv(ann, "ann_IP_CD4.csv", row.names = FALSE)
```
...
library(ComplexHeatmap)
col_ha <- HeatmapAnnotation(CAR = dds$Class1, Patient = dds$Patient_id)
Heatmap(gsva_results, show_column_names = FALSE, top_annotation = col_ha, row_dend_reorder = TRUE)
gsva_results_CD4 <- gsva_results
normalized_counts_CD4 <- normalized_counts
```
...
```{r , fig.height=8, fig.width=8}
# rm(list = ls())
rm(list = setdiff(ls(), c("ensembl", "integrated.obj", "my_annotations", "gsva_results_CD4", "normalized_counts_CD4")))

# Load data
# integrated.obj<-readRDS("integrated.obj.rds")

# Filter by IP
integrated.obj.subset <- subset(x = integrated.obj, subset = final_criteria == "CD8" & Timepoint == "IP")
...
ann$Analisis <- "CD8"
write.csv(ann, "ann_IP_CD8.csv", row.names = FALSE)
```
...
col_ha <- HeatmapAnnotation(CAR = dds$Class1, Patient = dds$Patient_id)
Heatmap(gsva_results, show_column_names = FALSE, top_annotation = col_ha, row_dend_reorder = TRUE)
normalized_counts_CD8 <- normalized_counts
```
